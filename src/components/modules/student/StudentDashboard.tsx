...
export default function StudentDashboard({ metadata }: Props) {
    /* ---------- MERGED REAL DATA ---------- */

    const stats = {
        totalCourses:
            metadata?.courses?.enrolled ?? mockData.stats.totalCourses,
        completedCourses:
            metadata?.courses?.completed ?? mockData.stats.completedCourses,
        wishlist: metadata?.courses?.wishlist ?? 0,
        totalSpent: metadata?.finance?.totalSpent ?? 0,
        hoursSpent: mockData.stats.hoursSpent,
        streakDays: mockData.stats.streakDays,
        avgScore: mockData.stats.avgScore,
        upcomingDeadlines: mockData.stats.upcomingDeadlines,
    };

    const recentCourses = mockData.recentCourses;
    const upcomingLessons = mockData.upcomingLessons;
    ...
    return (
        <div className="min-h-screen bg-linear-to-b from-gray-50 to-white p-4 md:p-6 lg:p-8">
            ...
            <div className="mb-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Button className="bg-linear-to-r from-blue-600 to-indigo-600 gap-2">
                ...
                <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                    <StatCard title="Learning Streak" value={`${stats.streakDays} days`} icon={<TrendingUp />} />
                    <StatCard title="Hours Spent" value={stats.hoursSpent} icon={<Clock />} />
                    <StatCard title="Avg Score" value={`${stats.avgScore}%`} icon={<Star />} />
                    <StatCard title="Courses Enrolled" value={stats.totalCourses} icon={<BookOpen />} />
                </div>
            </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="flex items-center justify-between text-sm text-gray-500">
                                                        ...
                                                        <div className="text-blue-600 font-medium">Next: {course.nextLesson}</div>
                                                    </div>
                                                </div>
                        ...
                        <CardContent>
                            <div className="space-y-3">
                                {achievements.map((achievement) => {
                                    ...
                                    return (
                                        <div key={achievement.id} className={`flex items-center gap-3 p-3 rounded-lg transition-all ${achievement.unlocked ? 'bg-linear-to-r from-amber-50 to-yellow-50 border border-amber-200' : 'bg-gray-50 opacity-60'}`}>
                        ...
                        <CardContent className="space-y-4">
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-semibold text-gray-900">{stats.totalCourses - stats.completedCourses}</span>
                                </div>
                                <div className="flex items-center justify-between">
            ...
            {/* REMAINING UI: COURSES, LESSONS, ACHIEVEMENTS, STATS */}
            {/* ⛔ EXACT SAME JSX YOU ALREADY HAVE */}
            {/* (Trimmed here for readability – your original JSX continues unchanged) */}
        </div>
    );
}

/* ---------- Small helper component (NO DESIGN CHANGE) ---------- */
